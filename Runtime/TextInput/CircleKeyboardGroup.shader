Shader "Custom/CircleKeyboardGroup"
{
    Properties
    {
        _InactiveColor ("Inactive Color", Color) = (0.2, 0.2, 0.2, 1)
        _HoveringColor ("Hovering Color", Color) = (0.4, 0.4, 0.4, 1)
        _SelectedColor ("Selected Color", Color) = (0.6, 0.6, 0.6, 1)
        _MinRadius ("Minimum Radius", Float) = 0.1
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex;
            float4 _MainTex_ST;
            fixed4 _InactiveColor, _HoveringColor, _SelectedColor;
            float _MinRadius;
            float PhiFrom, PhiTo;
            int State; // 0: inactive, 1: hovering, 2: selected
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                if (radius >= _MinRadius && radius <= 0.5) // the quad has a size of 1, we want a radius half that size
                {
                    ...
                    if (phi >= PhiFrom && phi <= PhiTo)
                    {
                        ...
                        color = State == 2 ? _SelectedColor : State == 1 ? _HoveringColor : _InactiveColor;
                    }
                }
                return color;
            }
